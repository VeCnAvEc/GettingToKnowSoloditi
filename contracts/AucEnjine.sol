...
contract AucEnjine {
    ...
    event AuctionCrated(uint index, string itemName, uint startingPrice, uint duraction);
    ...
    function createAuction(uint _startingPrice, uint _discountRate, string calldata _item, uint _duration) external {
            uint duraction = _duration == 0 ? DURATION : _duration;

            require(_startingPrice >= _discountRate * _duration, "inncorrect starting price");
            ...
            emit AuctionCrated(auctions.length - 1, _item, _startingPrice, duraction);
    }

    function getPriceFor(uint index) public view returns(uint) {
        Auction memory cAuction = auctions[index];
        require(!cAuction.stopped, "Sttoped!");
    ...
    function buy(uint index) external payable {
        Auction storage cAuction = auctions[index];
        require(!cAuction.stopped, "Stoped");
